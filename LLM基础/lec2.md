...
## 重点概念MFU Model FLOPs Utilization
定义
>[!info]
>MFU = (actual FLOP/s) / (promised FLOP/s)
>这个指大于 0.5就已经很不错了

bf16 或者是混合精度的模型一般情况下 MFU会比较小。这是因为模型的promised FLOP比较大。然而实际上由于 batch size 等原因，实际的 FLOP/s 会和 promised差很多。结果就是MFU比较小。

>[!success] 现在我们回答一下 为什么开头的时候 `total_flops = 6 * 70e9 * 15e12`
>一次 **forward**：每个参数贡献 2 FLOPs（乘法 + 加法）。
>
>反向传播时，由于 **链式法则**，需要同时计算：
>1. **w.r.t activations**：把梯度往前传 → 2 FLOPs  
>2. **w.r.t weights**：计算参数的梯度 → 2 FLOPs  
>
>所以 **backward = 4 FLOPs**。  
>
>最终：
>$$
\text{Total FLOPs} = (2+4)\,N_{\text{params}}\,N_{\text{tokens}}
= 6\,N_{\text{params}}\,N_{\text{tokens}}
$$



后面的模型实例，和具体的梯度运算，可以去看Slides。
Model 部分包括参数初始化和模型架构，这部分 COMP5329已经学过了